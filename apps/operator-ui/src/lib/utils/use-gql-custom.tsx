...
const getOperationNameFromQuery = (gqlQuery: DocumentNode): string | undefined => {
  const operationDef = gqlQuery.definitions.find((def) => def.kind === 'OperationDefinition') as
...
export const useGqlCustom = <T extends BaseRecord>({
  ...
  if (!operation) {
    throw new Error('Operation name could not be inferred from the GraphQL query.');
  }
